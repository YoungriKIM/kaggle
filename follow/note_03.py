...
test_df[TARGET] = pd.read_csv("D:/kaggle/sub/pseudo_label.csv")[TARGET]
...
classifier = XGBClassifier(n_estimators=1000, objective="reg:squarederror", use_label_encoder=False) # , device = 'gpu', LGBMClassifier
...
submission['submit_2'] = pd.read_csv("D:/kaggle/ensemble/dae.csv")[TARGET]
submission['submit_3'] = pd.read_csv("D:/kaggle/sub/pseudo_label.csv")[TARGET]

submission[[col for col in submission.columns if col.startswith('submit_')]].sum(axis = 1).value_counts()

submission[TARGET] = (submission[[col for col in submission.columns if col.startswith('submit_')]].sum(axis=1) >= 2).astype(int)

submission[['PassengerId', TARGET]].to_csv("D:/kaggle/sub/ensemble_04.csv", index = False)
